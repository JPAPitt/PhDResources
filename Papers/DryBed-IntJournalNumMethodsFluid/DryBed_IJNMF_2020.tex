...
%==========================
% Turn off or on the commenting
%==========================
\setboolean{showhighlight}{true}
...
\ifthenelse{\boolean{showhighlight}}{
...
Since \hlb{$h^\pm_{j\mp 1/2}$ and $G^\pm_{j\mp 1/2}$} use the same reconstruction operators we will demonstrate them for a general quantity \hlb{$q^\pm_{j\mp 1/2}$}. The values of $q$ are reconstructed at $x_{j-1/2} $, $x_{j} $ and $x_{j+1/2}$ from the cell averages $\overline{q}_j$ using the generalised minmod limiter \cite{VanLeer-1979-101}
%cite someone
	\begin{align}
...
\end{equation}
with $\theta \in \left[1,2\right]$. Note that although $h$ and $G$ have three reconstruction locations in Figure \ref{fig:ReconLocs}, the resultant reconstructed function is a linear polynomial as the reconstructed nodal value is the average of the cell edge values. \hlb{Linear reconstruction is sufficient to produce a second-order scheme.}
